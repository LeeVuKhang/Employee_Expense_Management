...
import { mockRequests, CURRENT_USER_ID, STATUS_CONFIG } from "../data/mockRequests";
...
export default function MyRequests() {
  ...
  // AC1: Only current user's requests
  const myRequests = useMemo(
    () => mockRequests.filter((r) => r.ownerId === CURRENT_USER_ID),
    []
  );

  const stats = useMemo(() => {
    const total = myRequests.length;
    const pending = myRequests.filter((r) => r.status === "pending_manager" || r.status === "pending_finance").length;
    const paid = myRequests.filter((r) => r.status === "paid").length;
    const rejected = myRequests.filter((r) => r.status === "rejected").length;
    const totalAmount = myRequests.filter((r) => r.status !== "rejected").reduce((s, r) => s + r.amount, 0);
  ...
  const filtered = useMemo(() =>
    myRequests.filter((r) => {
      ...
      const matchSearch = !q || r.id.toLowerCase().includes(q) || r.category.toLowerCase().includes(q) || r.description.toLowerCase().includes(q);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F9FAFB", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "36px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "center" }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, backgroundColor: "#FFFFFF", border: "1px solid #E5E7EB", borderRadius: 10, padding: "10px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by ID, category, or description..." style={{ flex: 1, border: "none", outline: "none", fontSize: 14, color: "#374151", backgroundColor: "transparent" }} />
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ padding: "10px 16px", border: "1px solid #E5E7EB", borderRadius: 10, fontSize: 14, color: "#374151", backgroundColor: "#FFFFFF", cursor: "pointer", outline: "none", fontFamily: "inherit", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", minWidth: 160 }}>
      ...
      {/* Detail Modal */}
      {selectedRequest && (
        <div onClick={() => setSelectedRequest(null)} style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 24 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: "#FFFFFF", borderRadius: 16, padding: 32, maxWidth: 480, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            {[
              ...
              ["Trip Dates", selectedRequest.tripDateFrom === selectedRequest.tripDateTo ? selectedRequest.tripDateFrom : `${selectedRequest.tripDateFrom} → ${selectedRequest.tripDateTo}`],
            ].map(([label, val]) => (
