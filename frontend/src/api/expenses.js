...
export async function fetchManagerPendingRequests() {
  ...
  const response = await requestExpense(
    `${managerApiRoutes.pendingRequests}?${params}`,
  );
  return normalizeManagerPendingRequests(response.requests ?? []);
}

// Manager-specific detail endpoint is available via managerApiRoutes.requestDetail

export async function fetchManagerPendingRequestsSummary() {
  const response = await requestExpense(
    managerApiRoutes.pendingRequestsSummary,
  );
...
export async function updateManagerExpenseRequestStatus(
  ...
  const expense = await requestExpense(
    managerApiRoutes.requestStatus(expenseId),
    {
      method: "PATCH",
      body: JSON.stringify({
        status,
...
function toManagerPendingRequestViewModel(request) {
  return {
    ...
    currentProcessorId:
      request.current_processor_id ?? request.currentProcessorId,
    requestType:
    ...
    amount: Number(
      request.total_amount ?? request.totalAmount ?? request.amount ?? 0,
    ),
    createdDate:
      toDateOnly(
        request.created_at ?? request.createdDate ?? request.submittedOn,
      ) ??
      request.start_date ??
      "",
    tripDateFrom:
      request.start_date ?? request.tripDateFrom ?? request.tripStart ?? "",
